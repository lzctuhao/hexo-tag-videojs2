...
(function (global, factory) {
    ...
    const onPlayerReady = (player, options) => {
      ...
      LongTab(player.touchOverlay.el_);
...
function LongTab(overlay_ele){
    if(!video_toast) {video_toast=function(){return};console.warn("Cannot find function video_toast. Please include <code>mytoast.js</code> in your html.")}
    ...
    addTabListener(
        ...
        function(){ //callback1
            ...
            document.querySelector(".vjs-touch-overlay").classList.remove('show-play-toggle');
        }, 
        function(){ //callback2
        ...
        rate_volume_brightness //callback3
    );
    ...
    function addTabListener(target,callback1,callback2,callback3){
        ...
        target.ontouchstart=(event)=>{
            rate_or_volume_or_brightness=0;is_longtab=false;timer=0; //初始化
            ...
            event.preventDefault();
            timer=setTimeout(()=>{is_longtab=true;callback1();timer=0},450)  // 超时器能成功执行，说明是长按
        }
        ...
        target.ontouchend=()=>{   // 到这里如果timer有值，说明此触摸时间不足450ms，是点击
        if(timer){clearTimeout(timer);}
            else{callback2()}
        }
    }

    function rate_volume_brightness(overlay_ele, w,nowX,spanX , h,nowY,spanY){ //callback3
        ...
        function change_rate(){
            ...
            let newTime=spanx_percentage*duation*0.8;
        ...
        switch(rate_or_volume_or_brightness){
            case 0:
                if (Math.abs(spanx_percentage)>0.1) change_rate()
                else if (nowx_percentage>0.67 && Math.abs(spany_percentage)>0.1) change_volume();

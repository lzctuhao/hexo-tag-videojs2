...
let has_videojs_toast=false;let origin_time;let video_ele;
//长按倍速播放
function LongTab(overlay_ele){
    //console.log(overlay_ele);
    !has_videojs_toast && overlay_ele.parentElement.insertAdjacentHTML('beforeend',"<div id='videojs-toast'><span id='videojstoast'></span></div>");
    video_ele = overlay_ele.parentElement.getElementsByTagName('video')[0];
    let rate=1;
    addLongtabListener(
        overlay_ele,
        function(){rate=video_ele.playbackRate;video_ele.playbackRate=3},
        function(){video_ele.playbackRate=rate;document.getElementById("videojstoast").style.opacity=0;},
        forward
    )
}
function forward(overlay_ele,spanx_rate){
    ...
    //document.getElementsByClassName("vjs-current-time-display")[0].innerHTML=durationTrans(video_ele.currentTime)
    document.getElementById("videojstoast").innerHTML=durationTrans(video_ele.currentTime);
    document.getElementById("videojstoast").style.opacity=1;
...
function addLongtabListener(target,callback1,callback2,callback3){/*目标dom，按下回调，滑动回调，长按结束回调 */
    let startX;
    let dom_w=target.clientWidth;
    let timer=0   // 初始化timer
    target.ontouchstart=(event)=>{
        startX = event.touches[0].pageX;
        origin_time=video_ele.currentTime;
        timer=0   // 重置timer
        timer=setTimeout(()=>{callback1();timer=0},1000)  // 超时器能成功执行，说明是长按
    }
    target.ontouchmove=(event)=>{
        clearTimeout(timer);    // 如果来到这里，说明是滑动
        timer=0;
        let spanX = event.changedTouches[0].pageX - startX;
        callback3(target,spanX/dom_w);
    }
    target.ontouchend=()=>{   // 到这里如果timer有值，说明此触摸时间不足500ms，是点击
